...
function HomePage() {
  ...
  let renderingCountries = allCountries
    .filter(
      (country) =>
        country?.region?.toLowerCase() === regionQuery || regionQuery === ""
    )
    .filter(
      (country) =>
        country?.subregion?.toLowerCase() === subRegionsQuery ||
        subRegionsQuery === ""
    )
    .filter((country) =>
